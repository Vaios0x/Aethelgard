...
export default function Drawer({ open, title, onClose, children }: DrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-0 top-0 h-full w-full sm:w-[420px] bg-surface border-l border-white/10 p-4 overflow-auto">
        <div className="flex items-center justify-between mb-3">
          <h2 className="heading text-xl">{title}</h2>
          <button className="btn-ghost px-3 py-1" onClick={onClose}>Cerrar</button>
        </div>
        {children}
      </div>
    </div>
